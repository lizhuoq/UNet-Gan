...
class DoubleConvolution(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, configs):
        ...
        """
        super().__init__()
        padding = (configs.kernel_size - 1) // 2
        # First $3 \times 3$ convolutional layer
        self.first = nn.Conv2d(in_channels, out_channels, kernel_size=configs.kernel_size, padding=padding)
        self.act1 = nn.ReLU()
        # Second $3 \times 3$ convolutional layer
        self.second = nn.Conv2d(out_channels, out_channels, kernel_size=configs.kernel_size, padding=padding)
        self.act2 = nn.ReLU()
